...
LATEST_RUN=$(ls -t logs/flow_swin_3plane/runs/ | head -1)
if [ -z "$LATEST_RUN" ]; then
    echo "No run directory found under logs/flow_swin_3plane/runs/"
    exit 1
fi

RUN_DIR="logs/flow_swin_3plane/runs/${LATEST_RUN}"
...
# 6) 把 evaluation_3plane.py 生成的图片/视频也收纳进该 run 目录（便于打包与复现）
#    evaluation_3plane.py 的默认输出目录：evaluation_results/evaluation_results_<run_timestamp>
