...
from evaluation_modules.flow_evaluator_3plane import Flow3PlaneEvaluator  # noqa: E402
...
def create_3plane_monitor_points():
    """Create 3-plane specific monitoring points - 27 points total (3 planes × 9 positions)."""
    ...
    # Create monitoring points for all 3 planes: (plane_idx, z_idx, x_idx)
    custom_points = []
    for plane_idx in range(3):  # planes 0, 1, 2
        for z_idx, x_idx in base_positions:
            custom_points.append((plane_idx, z_idx, x_idx))
...
def run_comprehensive_3plane_evaluation(
    ...
    # Determine monitoring points for 3-plane
    if custom_points:
        monitor_points = create_3plane_monitor_points()
        print(f"Using {len(monitor_points)} custom 3-plane monitoring points")
    else:
        # Assume 3-plane data uses similar spatial dimensions
    ...
    # Create 3-plane evaluator
    evaluator = Flow3PlaneEvaluator(
    ...
    try:
        ...
        for split in dataset_splits:
            ...
            for sample_idx in range(actual_num_samples):
                ...
                evaluator.evaluate_3plane_sample(sample_idx=sample_idx, split=split, num_future=num_future_steps)

        # Create comprehensive 3-plane analysis
        print(f"\n{'=' * 60}")
        print("📈 3-PLANE ANALYSIS PHASE")
        print(f"{'=' * 60}")

        evaluator.create_comprehensive_3plane_analysis()
        ...
        print(f"  - Monitored {len(monitor_points)} 3-plane points")
        ...
        print("  - Created comprehensive 3-plane metrics and visualizations")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Use custom 3-plane monitoring points instead of default grid",
    )
    ...
    parser.add_argument(
        "--output-dir", type=str, default="evaluation_3plane_outputs", help="Output directory for all results"
    )
    ...
    print(f"🔧 Starting 3-plane evaluation with checkpoint: {args.checkpoint_path}")
